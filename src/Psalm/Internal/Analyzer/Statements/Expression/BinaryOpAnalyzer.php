...
class BinaryOpAnalyzer
{
    ...
    public static function analyze(
        ...
        if ($stmt instanceof PhpParser\Node\Expr\BinaryOp\Concat && $nesting > 20) {
            ...
            $context_clauses = array_merge($context->clauses, $left_clauses);

            if ($context->reconciled_expression_clauses) {
            ...
            if ($context->parent_context) {
                ...
                foreach ($right_context->vars_in_scope as $var_id => $type) {
                    if (!isset($parent_context->vars_in_scope[$var_id])) {
                        $parent_context->vars_in_scope[$var_id] = $type;
                    }
                }

                $parent_context->referenced_var_ids = array_merge(
                    $context->referenced_var_ids,
                    $parent_context->referenced_var_ids
                );

                $parent_context->assigned_var_ids = array_merge(
                    $context->assigned_var_ids,
                    $parent_context->assigned_var_ids
                );

                if ($context->collect_references) {
                    $parent_context->unreferenced_vars = $context->unreferenced_vars;
                }
            }
